...
if __name__ == '__main__':
    ...
    start = time.time()
    model = SVR()
    print(str(y_ls) + str(y_ls.dtype) + "xtype" + str(X_ls.dtype))
    ...
    fname = make_submission(y_pred, test_user_movie_pairs, 'svm_C=1; eps = 0.2; gamma = scale')
    print('Submission file "{}" successfully written'.format(fname))
    print("error on LS: " + str(asses_model(model.predict(X_ls),y_ls)))
